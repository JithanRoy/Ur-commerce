...
- On a detail page, the customer picks option values (Size: 40, Colour: Black)
  ...
  [05-storefront-spec.md](05-storefront-spec.md#product-detail-page).
...
> ⚠️ **Known backend issue — this will fail from a browser today.**
> `main.ts` sets CORS `allowedHeaders: ["Content-Type", "Authorization"]`,
> which omits `X-Cart-Session` and `X-Tenant-Host`. The preflight rejects both,
> so guest carts and the dev tenant override break in the browser while working
> fine in curl/Postman. The fix is one line in `src/main.ts`:
> ```ts
> allowedHeaders: ["Content-Type", "Authorization", "X-Cart-Session", "X-Tenant-Host"],
> ```
> Confirm this has landed before you start on the cart.
